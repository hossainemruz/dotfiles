...
Read and follow `~/.config/agent-workflows/task-artifacts.md` before proceeding.

Run only on explicit user invocation after `/review-pr`. Run `taskctl context`; require `review.md` to identify the current PR/branch and contain actionable findings. Run `taskctl step get` and require the single corrective Step created by `/review-pr`; run `taskctl step start` if pending, continue if in progress, or, if ready, apply explicit feedback via `taskctl step revise` and continue; otherwise stop for `/accept-step`.
