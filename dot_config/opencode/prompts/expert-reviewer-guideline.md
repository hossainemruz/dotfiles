...
- For a Task PR review, run `taskctl pr context` once. Require a branch-associated completed current PR and treat the returned projection as the working contract. Do not read all of `plan.md` when the projection contains the required requirements, PR, Step, and artifact context.
