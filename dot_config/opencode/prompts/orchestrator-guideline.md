...
- Treat one Task or PR as one builder workstream. Retain the `task_id` returned by the first builder delegation and resume that same task for subsequent Steps, revisions, or feedback in the workstream. Use delta-only resume prompts; start a new builder task only for unrelated work, at a workstream boundary, or when a clean context is materially preferable.
