...
- Never start or dispatch the next planned implementation Step because a command or subagent reports or recommends it as the next action. Starting that Step requires a new, explicit user invocation of `/next-step` or `/next-step-hard`; `/accept-step` only accepts the current Step and then stops. The sole exception is the corrective Step created by a completed-PR `/review-pr` or `/expert-review` workflow with actionable findings, which the automatic review-remediation rule starts immediately.
...
- You own review routing, but builders already self-review each Step. For `taskctl` PR work, never invoke `@reviewer` or `@expert-reviewer` after an individual Step is submitted or accepted. Once every planned implementation Step in the PR is complete, you may automatically invoke one full PR-scale review or run it in response to an explicit `/review-pr` or `/expert-review` request. For non-`taskctl` work, use `@reviewer` only for risky or behavior-changing ad hoc diffs; use `@expert-reviewer` for explicit PR-level review or high-risk release gates.
- After a PR-scale `/review-pr` or `/expert-review` workflow—whether invoked automatically or explicitly—returns actionable findings, immediately start one fresh builder workstream to address all findings instead of only reporting them. For a Task-backed review that created a corrective Step and `review.md`, follow the `/address-review` contract: start or continue that Step, address every finding, validate, self-review, and submit it without completing it. For an ad hoc `/expert-review`, pass the complete findings as the builder's bounded contract, fix them, validate, and self-review without invoking `taskctl`. If the review approves with no actionable findings, do not dispatch a builder. Ordinary `/review` remains review-only and never triggers automatic remediation.
