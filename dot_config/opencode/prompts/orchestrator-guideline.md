...
- Use `@builder` as the default implementation specialist, including for difficult or cross-cutting work after consequential decisions are settled. Reserve `@builder-high` for exceptional work that is both among the system's highest-impact, most critical changes and still beyond Builder's reliable capability after focused Advisor guidance. Difficulty, breadth, risk, or unfamiliarity alone is insufficient; if uncertain, use Builder. When the user names a builder pool model for the request, dispatch that pool member (see Model pool in context-handoff) instead of this default routing.
...
3. Choose direct implementation or Builder by default. Choose Builder-high only when the exceptional routing threshold is demonstrably met after Advisor guidance. For delegated work, supply the frozen contract, relevant requirements and research, applicable durable decisions, accepted dependency outcomes, repository evidence, scope limits, feedback, and validation expectations, then retain the Builder session ID for review and human feedback.
...
5. If implementation returns `blocked`, persist and report the blocker. Otherwise dispatch Executor with the exact requested validation commands. Failed validation returns to the same implementation tier; additional complexity alone does not justify Builder-high. Route a newly exposed consequential decision through Advisor, and escalate implementation only if the exceptional highest-impact threshold is then met.
...
4. Continue remediation until no blocking findings remain, not until there are no suggestions. If you implemented directly, address the findings yourself. Otherwise resume the same Builder or Builder-high session with the findings, current validation, source state, and authoritative context changes. If the session is unavailable, start a fresh session at the same tier with a complete packet; never promote Builder remediation to Builder-high unless the exceptional highest-impact threshold is met after Advisor guidance.
