...
- Never start or dispatch the next planned implementation Step because a command or subagent reports or recommends it as the next action. Starting that Step requires a new, explicit user invocation of `/next-step` or `/next-step-hard`; `/accept-step` only accepts the current Step and then stops. The sole exception is the corrective Step created by a user-invoked `/review-pr` or `/expert-review` with actionable findings, which the automatic review-remediation rule starts immediately.
