...
Run only on explicit user invocation after `/review-pr`. Run `taskctl step context` exactly once and use its projected requirements, PR, corrective Step, and artifact paths as the working contract; do not also run `taskctl context`, `taskctl step get`, or read all of `plan.md` by default. Require `review.md` to identify the current PR/branch and contain actionable findings, and require the single corrective Step created by `/review-pr`; run `taskctl step start` if pending, continue if in progress, or, if ready, apply explicit feedback via `taskctl step revise` and continue; otherwise stop for `/accept-step`.
