...
Then ask `@expert-reviewer` to review the full diff against base and write
artifact `review.md`, preserving its structure. `@expert-reviewer` may use
`@executor` for execution-heavy validation, but must not delegate review.
