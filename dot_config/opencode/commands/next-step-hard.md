---
description: Select the next PR and implement a high-risk Step with extended reasoning.
agent: orchestrator
subtask: false
---
...
Dispatch one fresh `@builder-high` workstream for this Step (resume only a known retry/feedback workstream for this same Step). Pass the complete raw projection, exact IDs, artifact paths, requirements, `$ARGUMENTS`, validation expectations, and relevant working-tree context. If it returns `status: ready_to_submit`, run `taskctl step submit` and trust successful output without refreshing. If blocked, report the exact blocker. Never complete without explicit acceptance. Report PR/Step, files, validation, self-review, lifecycle status, and next action.
