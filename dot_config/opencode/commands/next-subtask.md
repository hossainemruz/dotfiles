...
Read `.work/plan.md` and select exactly one active sub-task using this order:
...
3. ask the `@reviewer` subagent to review the changes against the active sub-task and write feedback to `.work/review.md`
4. fix any issues you agree with
5. ask `@executor` to rerun the relevant validation after fixes
6. update `.work/plan.md` to mark the sub-task `Completed`
